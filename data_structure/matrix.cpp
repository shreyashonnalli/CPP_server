...
std::ostream& operator<<(std::ostream& os, const SparseMatrix& matrix){
    ...
    for(const auto& pair: matrix.matrix_){
        os << "(" << pair.first.first << "," << pair.first.second << ")" << " = " << pair.second << "\n";
    }
    return os;
}

void SparseMatrix::set(uint32_t row, uint32_t col, char value) {
    auto key = getKey(row, col);
    if (!matrix_.count(key)) matrix_[key] = value;
}

char SparseMatrix::get(uint32_t row, uint32_t col) {
    auto key = getKey(row, col);
    if (!matrix_.count(key)) return '\0';
    else return matrix_[key];
}

bool SparseMatrix::isReadValid(uint32_t startRow, uint32_t startCol){
    if (startRow < GRID_SIZE && startCol < GRID_SIZE) return true;
    else return false;
}
...
bool SparseMatrix::isInsertValid(uint32_t startRow, uint32_t startCol, std::string direction, u_long writeSize){
    if (startRow == GRID_SIZE || startCol == GRID_SIZE) return false;
    ...
    if (resultingColPtr >= -1 && resultingColPtr <= gridBound && resultingRowPtr >= -1 && resultingRowPtr <= gridBound) return true;
    return false;
}


int SparseMatrix::insertString(uint32_t startRow, uint32_t startCol, std::string direction, std::string toInsert){
    if (!isInsertValid(startRow, startCol, direction, toInsert.length())) return -1;
