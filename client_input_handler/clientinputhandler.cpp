...
bool ClientInputHandler::validateClientInput(const char *message)
{
    ...
    if (components.size() == 6)
        return validateReadInput(components);
    else
        return validateWriteInput(components);
}

// divides into components
...
bool ClientInputHandler::validateReadInput(std::vector<std::string> components)
{
    ...
    if (components[0] != "READ")
        return false;
    if (!checkIsUINT64(components[1]))
        return false;
    if (!checkIsUINT32(components[2]))
        return false;
    if (!checkIsUINT32(components[3]))
        return false;
    if (!checkIsDirection(components[4]))
        return false;
    if (components[5] != "END")
        return false;
    return true;
...
// assumes valid input. Called after checking validity
...
// get various parts of a component
